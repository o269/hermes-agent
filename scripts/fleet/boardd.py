...
WHY THIS EXISTS
---------------
The legacy fleet mirror under a user-owned Hermes home reliably
...
quarantine files are this board. The single-writer default board stays CLEAN.
A cross-process advisory flock that
...
DEFAULT_DB = os.environ.get(
    "BOARDD_DB",
    "/var/lib/boardd/fleet/kanban.db",
)
...
def _canon_reasoning_effort(value):
    """Validate a broker-native task's optional thinking-depth override."""
    ...
    if normalized not in _REASONING_EFFORTS:
        raise ValueError(
            ...
            f"{', '.join(sorted(_REASONING_EFFORTS))}, got {value!r}"
        )
    return normalized
...
class Broker:
    def __init__(
        ...
        *,
        ...
        self._assignment_policy_ready = False
        self._assignment_authority_profiles = []
    ...
    def _open_conn(self) -> sqlite3.Connection:
        ...
        if self.import_schema:
            ...
            self._assignment_authority_profiles = list(k._authority_profiles(conn))
        ...
        # ``kanban_db.connect()`` installs the policy guard/SQLite triggers for
        # the canonical ``--import-schema`` runtime.  A custom schema source is
        # intentionally not treated as policy-ready: test/migration brokers may
        # omit the current tasks schema and must not imply a live authority
        # fence that is not present.
    ...
    def _health(self) -> dict:
        ...
        return {
            ...
            "assignment_policy_ready": self._assignment_policy_ready,
            "assignment_authority_profiles": self._assignment_authority_profiles,
            "write_canary_ok": (
                None
