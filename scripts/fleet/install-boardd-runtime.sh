...
for required in \
  ...
  "$SOURCE_ROOT/hermes_cli/kanban_assignment_policy.py" \
...
for tracked in \
  ...
  hermes_cli/kanban_assignment_policy.py \
...
if [[ ! -d "$release_dir" ]]; then
    ...
    "$stage_dir/venv/bin/python" "$stage_dir/libexec/boardd.py" --help >/dev/null
  {
    ...
    printf 'kanban_assignment_policy_sha256=%s\n' "$(sha256sum "$SOURCE_ROOT/hermes_cli/kanban_assignment_policy.py" | cut -d' ' -f1)"
