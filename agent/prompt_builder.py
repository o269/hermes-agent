...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**ABSOLUTE PROHIBITION (operator order 2026-08-03): NEVER open the board "
    "database file directly — no `sqlite3` CLI, no `sqlite3.connect()`, and "
    "especially never via `sudo`. Root-level direct opens spawn a second WAL, "
    "corrupt the board (11 corruption incidents traced to exactly this), and a "
    "root fence process now SIGKILLs any non-broker holder of the board file "
    "within seconds. If your `kanban_*` tools are missing or erroring, that is "
    "a BLOCKING condition: call `kanban_block(reason=...)` if you can, otherwise "
    "stop and report — do not improvise raw SQL access.**\n"
    "\n"
    "## Lifecycle\n"
    "\n"
