...
KANBAN_GUIDANCE = (
    ...
    "- **GitHub PRs.** Do not rely on `gh` repository defaults. For PR-producing "
    "code tasks, pass an explicit destination and base, for example "
    "`gh pr create --repo <owner/repo> --base main --head <branch>`. "
    "In Hermes-Agent workspaces the fleet-safe destination is "
    "`o269/hermes-agent`; public `NousResearch/Hermes-Agent` PRs require "
    "explicit task authorization for an upstream contribution.\n"
