...
def test_config_registry_matches_runtime_default():
    """The documented key must be canonical, not an unknown YAML extra."""
    from hermes_cli.config import _validate_config_key
    from hermes_cli.config_defaults import DEFAULT_CONFIG

    configured = DEFAULT_CONFIG["kanban"]["max_spawn_per_tick"]
    assert configured == kb.DEFAULT_MAX_SPAWN_PER_TICK
    assert _validate_config_key("kanban.max_spawn_per_tick") == (True, None)
...
_GATEWAY_UNSET = object()


@pytest.mark.parametrize(
    ("configured", "expected"),
    [
        (_GATEWAY_UNSET, kb.DEFAULT_MAX_SPAWN_PER_TICK),
        (7, 7),
        ("lots", kb.DEFAULT_MAX_SPAWN_PER_TICK),
        (0, None),
    ],
    ids=["default", "override", "invalid", "disabled"],
)
def test_gateway_wires_burst_fence_config(
    tmp_path, monkeypatch, configured, expected
):
    """The embedded gateway applies the same four config modes as the CLI."""
    from gateway import kanban_watchers as watchers
    from hermes_cli import config as live_config
    from hermes_cli import kanban_db as live_kb

    kanban_cfg = {
        "dispatch_in_gateway": True,
        "auto_decompose": False,
    }
    if configured is not _GATEWAY_UNSET:
        kanban_cfg["max_spawn_per_tick"] = configured
    monkeypatch.setattr(
        live_config, "load_config", lambda: {"kanban": kanban_cfg}
    )
    monkeypatch.setattr(
        watchers, "_acquire_singleton_lock", lambda path: (None, "unavailable")
    )
    monkeypatch.setattr(live_kb, "kanban_home", lambda: tmp_path)
    monkeypatch.setattr(
        live_kb, "kanban_db_path", lambda board=None: tmp_path / "kanban.db"
    )
    monkeypatch.setattr(
        live_kb,
        "list_boards",
        lambda *args, **kwargs: [{"slug": live_kb.DEFAULT_BOARD}],
    )
    monkeypatch.setattr(live_kb, "reap_worker_zombies", lambda: [])
    monkeypatch.setattr(live_kb, "has_spawnable_ready", lambda conn: False)
    monkeypatch.setattr(live_kb, "has_spawnable_review", lambda conn: False)
    ...
    monkeypatch.setattr(
        live_kb, "connect", lambda board=None: FakeConnection()
    )
    ...
    monkeypatch.setattr(live_kb, "dispatch_once", fake_dispatch_once)
    monkeypatch.setattr(watchers.asyncio, "to_thread", fake_to_thread)
    monkeypatch.setattr(watchers.asyncio, "sleep", fake_sleep)

    asyncio.run(runner._kanban_dispatcher_watcher())

    assert captured["max_spawn_per_tick"] == expected
