...
def _candidate_on_separate_test_filesystem(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Unit candidates model the operator-required separate archive mount."""

    candidate_device = tmp_path.stat().st_dev
    monkeypatch.setattr(
        cold,
...
def _board(tmp_path: Path, name: str = "stage") -> Path:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO tasks(id, status, session_id) VALUES (?, ?, ?)",
            ("t_control", "done", None),
        )
        conn.commit()
    finally:
...
def _producer(
    ...
    *,
    ...
    receipt = cold.run_cold_archive_pass(
        source_db=db_path,
        stage_root=stage,
        board_db=_board(tmp_path, stage_name),
        now=NOW,
        hold_sources=[],
...
def _apply(
    ...
    return cold.run_cold_archive_pass(
        source_db=db_path,
        stage_root=stage,
        board_db=_board(stage.parent, stage.name),
...
def test_each_required_canonical_policy_column_fails_closed_when_missing(
    ...
    with pytest.raises(
        ...
        cold.build_gate_b_manifest(
            db_path,
            board_db=_board(tmp_path, "missing-column"),
            now=NOW,
            hold_sources=[],
        )
...
def test_manifest_uses_canonical_activity_and_exact_37_day_boundary(
    ...
    manifest = cold.build_gate_b_manifest(
        db_path, board_db=_board(tmp_path, "boundary"), now=NOW, hold_sources=[]
    )
...
def test_every_nonterminal_or_unknown_board_status_holds_whole_lineage(
    tmp_path: Path, status_value: str
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    try:
        _make_session(db, "parent", days_ago=90, content="parent")
        _make_session(
            db,
            "child",
            days_ago=90,
        ...
        db.close()
    board_path = _board(tmp_path, "live-lineage")
    conn = sqlite3.connect(board_path)
    try:
        conn.execute(
            "INSERT INTO tasks(id, status, session_id) VALUES (?, ?, ?)",
            ("t_live", status_value, "child"),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    manifest = cold.build_gate_b_manifest(
        db_path, board_db=board_path, now=NOW, hold_sources=[]
    )

    assert manifest["_restricted_selected_ids"] == []
    assert manifest["skipped_group_reason_counts"] == {"live_board_task_reference": 1}
    assert manifest["board_liveness"]["unknown_statuses_are_live"] is True
    assert manifest["board_liveness"]["live_linked_tasks"] == 1


@pytest.mark.parametrize(
    "status_value", ["done", "archived", "completed", "cancelled", "canceled"]
)
def test_terminal_board_status_does_not_hold_cold_session(
    tmp_path: Path, status_value: str
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    try:
        ...
        db.close()
    board_path = _board(tmp_path, "terminal")
    conn = sqlite3.connect(board_path)
    try:
        conn.execute(
            "INSERT INTO tasks(id, status, session_id) VALUES (?, ?, ?)",
            ("t_terminal", status_value, "eligible"),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    assert manifest["_restricted_selected_ids"] == ["eligible"]
    assert manifest["board_liveness"]["live_linked_tasks"] == 0


@pytest.mark.parametrize(
    "board_shape",
    ["missing", "empty", "no-tasks", "missing-column", "blank-session-link"],
)
def test_board_liveness_source_errors_fail_before_stage_or_candidate_mutation(
    tmp_path: Path, board_shape: str
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    try:
        ...
        db.close()
    board_path = tmp_path / f"{board_shape}.db"
    if board_shape != "missing":
        conn = sqlite3.connect(board_path)
        try:
            if board_shape == "empty":
                conn.execute(
                    "CREATE TABLE tasks (id TEXT PRIMARY KEY, status TEXT, session_id TEXT)"
                )
            elif board_shape == "no-tasks":
                conn.execute("CREATE TABLE unrelated (id TEXT)")
            elif board_shape == "missing-column":
                conn.execute("CREATE TABLE tasks (id TEXT PRIMARY KEY, status TEXT)")
                conn.execute("INSERT INTO tasks(id, status) VALUES ('t', 'done')")
            else:
                conn.execute(
                    "CREATE TABLE tasks (id TEXT PRIMARY KEY, status TEXT, session_id TEXT)"
                )
                conn.execute(
                    "INSERT INTO tasks(id, status, session_id) VALUES ('t', 'ready', '   ')"
                )
            conn.commit()
        finally:
            conn.close()
    ...
    with pytest.raises(ColdArchiveError, match="board|tasks"):
...
def test_producer_rejects_cold_age_below_hard_37_day_floor_without_stage(
    ...
    with pytest.raises(ColdArchiveError, match="at least 37 days"):
        cold.run_cold_archive_pass(
            source_db=db_path,
            stage_root=stage,
            board_db=_board(tmp_path, "floor-producer"),
            now=NOW,
            hot_days=hot_days,
...
def test_apply_rejects_approved_policy_below_hard_37_day_floor_without_delete(
    ...
    try:
        ...
        db.close()
    manifest = cold.build_gate_b_manifest(
        db_path, board_db=_board(tmp_path, "floor-apply"), now=NOW, hold_sources=[]
    )
    ...
    with pytest.raises(ColdArchiveError, match="at least 37 days"):
        cold.apply_retention_to_candidate(
            db_path,
            manifest,
            board_db=_board(tmp_path, "floor-apply"),
...
def test_null_canonical_activity_or_pin_state_is_not_selectable(tmp_path: Path) -> None:
    ...
    manifest = cold.build_gate_b_manifest(
        db_path, board_db=_board(tmp_path, "nullable"), now=NOW, hold_sources=[]
    )
    assert manifest["_restricted_selected_ids"] == []
...
def test_nonfinite_canonical_activity_fails_closed(
    ...
    with pytest.raises(ColdArchiveError, match="must be a finite number"):
        cold.build_gate_b_manifest(
            db_path, board_db=_board(tmp_path, "nonfinite"), now=NOW, hold_sources=[]
        )
...
def test_rejects_default_named_profile_and_sidecar_hardlink_aliases(
    ...
    for index, protected in enumerate((default_db, named_db, sidecar)):
        ...
        with pytest.raises(ColdArchiveError, match="hardlink"):
...
def test_candidate_must_be_owned_single_link_and_on_separate_filesystem(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    monkeypatch.setattr(os, "geteuid", lambda: db_path.stat().st_uid + 1)
    with pytest.raises(ColdArchiveError, match="owned by the current effective user"):
        cold.reject_live_state_db(db_path)

    monkeypatch.setattr(os, "geteuid", lambda: db_path.stat().st_uid)
    ...
    source_device = db_path.stat().st_dev
    monkeypatch.setattr(
        cold, "_protected_live_filesystem_devices", lambda: frozenset({source_device})
    )
    with pytest.raises(ColdArchiveError, match="different filesystem"):
        cold.build_gate_b_manifest(
            db_path,
            board_db=_board(tmp_path, "boundary-refusal"),
            now=NOW,
            hold_sources=[],
        )
...
def test_existing_stage_is_untouched_and_never_chmodded_or_overwritten(
    ...
    with pytest.raises(ColdArchiveError, match="refusing existing stage path"):
        cold.run_cold_archive_pass(
            source_db=db_path,
            stage_root=stage,
            board_db=_board(tmp_path, "existing-stage"),
            now=NOW,
            hold_sources=[],
            manifest_only=True,
        )
...
def test_remote_preflight_drift_is_rejected_by_complete_approved_snapshot(
    ...
    with pytest.raises(
        ColdArchiveError,
        match="candidate (sha256|logical snapshot) differs from approved manifest",
    ):
...
def test_surviving_tool_message_is_preserved_and_uses_standard_fts_only(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    assert _readonly_rows(
        ...
        "WHERE messages_fts MATCH 'survivingtoolmatchtoken'",
    ) == [(1,)]
    assert _readonly_rows(
        db_path,
        "SELECT COUNT(*) FROM messages_fts_trigram "
        "WHERE messages_fts_trigram MATCH 'survivingtoolmatchtoken'",
    ) == [(0,)]
...
def test_survivor_fts_content_drift_rolls_back_and_preserves_match_semantics(
    ...
    with pytest.raises(
        ColdArchiveError, match="messages_fts strict content integrity check failed"
    ):
        _apply(db_path, stage, config, producer, fake)
...
def test_approved_snapshot_is_rechecked_after_connection_before_begin_immediate(
    ...
    with pytest.raises(
        ColdArchiveError,
        match="candidate (path bytes/identity|logical snapshot) changed before destructive lock",
    ):
        _apply(db_path, stage, config, producer, fake)
...
def test_selected_prompt_delete_failure_rolls_back_sessions_messages_fts_and_prompts(
    ...
    try:
        db._conn.execute(
            "INSERT INTO system_prompts(hash, prompt) VALUES ('selected-prompt', 'body')"
        )
...
def test_wrong_external_sha_is_rejected_and_external_frozen_copies_are_accepted(
    ...
    with pytest.raises(ColdArchiveError, match="exact-byte sha256 mismatch"):
        cold.run_cold_archive_pass(
            source_db=db_path,
            stage_root=stage,
            board_db=_board(tmp_path, "wrong-sha"),
            apply_retention=True,
    ...
    cold.run_cold_archive_pass(
        source_db=db_path,
        stage_root=stage,
        board_db=_board(tmp_path, "stage"),
        apply_retention=True,
...
def test_source_and_stage_symlinks_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ColdArchiveError, match="symlink path component"):
        cold.run_cold_archive_pass(
            ...
            board_db=_board(tmp_path, "link-source"),
            now=NOW,
            hold_sources=[],
            manifest_only=True,
        )
...
def test_each_required_fts_root_and_sync_trigger_fails_closed(
    ...
    with pytest.raises(
        ...
        cold.run_cold_archive_pass(
            ...
            board_db=_board(tmp_path, f"fts-{missing_object}"),
            now=NOW,
            hold_sources=[],
            manifest_only=True,
        )
...
@pytest.mark.parametrize("marker", ["fts_rebuild_high_water", "fts_rebuild_progress"])
def test_incomplete_fts_rebuild_fails_closed(tmp_path: Path, marker: str) -> None:
    db_path = tmp_path / f"candidate-{marker}.db"
    db = _build_archive_candidate(db_path)
    try:
        _make_session(db, "eligible", days_ago=90, content="selected")
        assert db._conn is not None
        db._conn.execute(
            "INSERT INTO state_meta(key, value) VALUES (?, ?)", (marker, "1")
        )
        db._conn.commit()
    finally:
        db.close()

    with pytest.raises(ColdArchiveError, match="incomplete FTS rebuild"):
        cold.run_cold_archive_pass(
            source_db=db_path,
            stage_root=tmp_path / f"stage-{marker}",
            board_db=_board(tmp_path, f"fts-{marker}"),
            now=NOW,
            hold_sources=[],
            manifest_only=True,
        )
...
def test_nonfinite_archive_windows_fail_before_stage_creation(
    ...
    with pytest.raises(ColdArchiveError, match="finite number"):
        cold.run_cold_archive_pass(
            source_db=db_path,
            stage_root=stage,
            board_db=_board(tmp_path, "nonfinite-window"),
            hot_days=bad,
            hold_sources=[],
            manifest_only=True,
        )
...
def test_encrypted_remote_rollback_decrypts_and_restores_snapshot(
    ...
    receipt = cold.run_cold_archive_pass(
        source_db=db_path,
        stage_root=stage,
        board_db=_board(tmp_path, "encrypted"),
        now=NOW,
        hold_sources=[],
...
def test_live_profile_hardlink_appearing_before_commit_rolls_back(
    ...
    with pytest.raises(ColdArchiveError, match="hardlink|active Hermes state.db"):
        _apply(db_path, stage, config, producer, fake)
    assert _readonly_rows(db_path, "SELECT id FROM sessions") == [("eligible",)]
    assert not (stage / "COLD-ARCHIVE-RETENTION-RECEIPT.json").exists()


def test_live_board_reference_appearing_after_producer_blocks_apply(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    fake = FakeRclone()
    stage, _recipient, config, producer = _producer(tmp_path, db_path, fake)
    board_path = _board(tmp_path, "stage")
    conn = sqlite3.connect(board_path)
    try:
        conn.execute(
            "INSERT INTO tasks(id, status, session_id) VALUES (?, ?, ?)",
            ("late-live", "ready", "eligible"),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    with pytest.raises(ColdArchiveError, match="non-terminal board task reference"):
        _apply(db_path, stage, config, producer, fake)
...
def test_apply_holds_board_writer_reservation_through_candidate_commit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    board_path = _board(tmp_path, "stage")
    original = cold._exclusive_write_json
    reservation_proven: list[bool] = []

    def probe_writer_reservation(path: Path, payload: dict[str, Any]) -> Path:
        if path.name == "COLD-ARCHIVE-APPLY-PREPARED.json":
            contender = sqlite3.connect(board_path, timeout=0, isolation_level=None)
            try:
                with pytest.raises(sqlite3.OperationalError, match="locked"):
                    contender.execute("BEGIN IMMEDIATE")
                reservation_proven.append(True)
            finally:
                contender.close()
        return original(path, payload)

    monkeypatch.setattr(cold, "_exclusive_write_json", probe_writer_reservation)
    receipt = _apply(db_path, stage, config, producer, fake)

    assert reservation_proven == [True]
    assert receipt["retention"]["board_writer_reservation_held_through_commit"] is True
    assert _readonly_rows(db_path, "SELECT id FROM sessions") == []
...
def test_rclone_config_must_be_private_and_unaliased(tmp_path: Path) -> None:
    ...
    with pytest.raises(ColdArchiveError, match="mode 0600"):
        cold.run_cold_archive_pass(
            ...
            board_db=_board(tmp_path, "config-mode"),
            now=NOW,
            hold_sources=[],
            rclone_remote=REMOTE,
            rclone_config=config,
    ...
    with pytest.raises(ColdArchiveError, match="hardlink aliases"):
        cold.run_cold_archive_pass(
            ...
            board_db=_board(tmp_path, "config-alias"),
            now=NOW,
            hold_sources=[],
            rclone_remote=REMOTE,
            rclone_config=config,
...
def test_external_recipient_and_rclone_config_are_frozen_before_tools_run(
    ...
    receipt = cold.run_cold_archive_pass(
        source_db=db_path,
        stage_root=stage,
        board_db=_board(tmp_path, "frozen-inputs"),
        now=NOW,
        hold_sources=[],
        rclone_remote=REMOTE,
        rclone_config=config,
...
def test_apply_rejects_hardlinked_stage_artifact_without_deleting(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    fake = FakeRclone()
    stage, _recipient, config, producer = _producer(tmp_path, db_path, fake)
    alias = tmp_path / "selected-ids-alias.json"
    os.link(stage / "restricted" / "selected-session-ids.json", alias)
...
def test_apply_rejects_tampered_restricted_board_liveness_receipt(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    fake = FakeRclone()
    stage, _recipient, config, producer = _producer(tmp_path, db_path, fake)
    board_ids = stage / "restricted" / "board-live-session-ids.json"
    board_ids.write_text(
        json.dumps({"live_board_session_ids": ["eligible"]}), encoding="utf-8"
    )
    os.chmod(board_ids, 0o600)

    with pytest.raises(ColdArchiveError, match="board-live session IDs"):
        _apply(db_path, stage, config, producer, fake)
...
def test_prune_release_to_reopen_sidecar_swap_restores_plaintext_and_authority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "candidate.db"
    db = _build_archive_candidate(db_path)
    ...
    def replace_sidecars_before_fresh_open(path: Path) -> sqlite3.Connection:
        if receipt_written and not replaced:
            ...
            for suffix in ("-wal", "-shm"):
                sidecar = db_path.with_name(db_path.name + suffix)
                replacement = tmp_path / f"release-gap-conflicting{suffix}"
                replacement.write_bytes(sidecar_bytes[suffix])
                os.chmod(replacement, 0o600)
                os.replace(replacement, sidecar)
                replaced.append(suffix)
    ...
    monkeypatch.setattr(cold, "_connect_candidate", replace_sidecars_before_fresh_open)
    with pytest.raises(
        ColdArchiveError,
        match="SQLite namespace changed before fresh prune verification",
    ):
        cold.prune_source_bundle_after_retention(
            stage,
            candidate_health_confirmed=True,
    ...
    result = subprocess.run(
        [
            ...
            (
                ...
                f"{str(tmp_path / 'missing.db')!r},'--board-db',"
                f"{str(tmp_path / 'board.db')!r},'--stage-root',"
...
def test_cli_help_requires_external_approval_flags_and_has_no_clock_override() -> None:
    ...
    assert "--board-db BOARD_DB" in result.stdout
