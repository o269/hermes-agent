...
The broker is part of the Kanban assignment-policy enforcement boundary. Its
systemd unit fixes `HERMES_HOME=/var/lib/boardd`; therefore the authority roster
must be configured in `/var/lib/boardd/config.yaml`, not in a worker profile's
home. A production `--import-schema` daemon reports
`assignment_policy_ready=true` in `health`, plus the roster it loaded in
`assignment_authority_profiles`; treat a false value or an unexpected roster as
a failed cutover and roll back before admitting clients.
