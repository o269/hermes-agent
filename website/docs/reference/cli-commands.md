...
| `decompose <id>` / `decompose --all` | Fan an eligible triage task out into a dependency-checked graph of child tasks routed only to profiles proven resolvable on a known host. Capability descriptions are the primary routing signal; configured profile provider/model values are defaults and tie-breaker context, never child-level overrides. An already-assigned root keeps its assignee and `triage` status; an unassigned root is assigned to a resolved orchestrator and promoted normally. The shared `fleet` board accepts named worker lanes only—the implicit root profile named `default` is not a routing fallback there. Control-plane gates, stale candidates, and cards that own an active PR are reported as skips without invoking replacement work; active-PR skips print the exact continuation review/authorize path. Successful output reports root status and dependency-closure counts. Falls back to custody-preserving specify behavior when fan-out is unnecessary. Same flags as `specify`. Configure the model under `auxiliary.kanban_decomposer` in `config.yaml`; `kanban.orchestrator_profile` never overrides existing custody. Also runs automatically every dispatcher tick when `kanban.auto_decompose: true` (the default). See [Auto vs Manual orchestration](/user-guide/features/kanban#auto-vs-manual-orchestration). |
| `continuation review\|authorize\|show <id>` | Read or mutate the one-shot active-PR repair ledger. `review` and `authorize` are control-plane mutations and must run as `/kanban` commands through the live root gateway. Authorization pins each `OWNER/REPO#N@SHA`, the task assignee/profile, provider, reason, and expiry. |
