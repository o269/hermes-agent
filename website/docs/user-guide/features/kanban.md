...
- **Declared** — the task's own work product (a run summary, run metadata, run error, or `tasks.result`) names the PR. Always guards.
- **Referenced** — the URL only appears in a comment on this task. If a *different* task has declared the same PR, the URL is a citation and does **not** guard; the dispatcher emits a `respawn_guard_pr_ignored` event naming the real owner. If no task anywhere has declared it, the conservative behaviour is preserved and it still guards (comment-only handoffs are common).
