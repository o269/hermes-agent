...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or the task owns an open GitHub PR (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).
...
Whenever the guard does fire, the `respawn_guarded` event payload carries `pr_url`, `source_comment_id`, `ownership`, and `expires_at`, so `hermes kanban tail <id>` explains the freeze without reading the database.
...
| `respawn_guarded` | `{reason}`, plus `{pr_url, pr_urls, ownership, source_comment_id, expires_at, window_seconds, ignored_pr_urls}` when the reason is `active_pr` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (this task owns a GitHub PR seen in the last 24 h — a prior worker already opened a PR). For `active_pr` the payload names the exact URL, the comment it came from, whether ownership was `declared` (the task's own work product or its own worker's comment) or `referenced` (cross-posted by another profile and unclaimed elsewhere), and the timestamp the window lapses. The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
