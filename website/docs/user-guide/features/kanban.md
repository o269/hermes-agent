...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `hermes kanban decompose <id> --idempotency-key <stable-key>` (or `--all --idempotency-key <stable-batch-key>`), or use `/kanban decompose <id> --idempotency-key <stable-key>` from a chat. Reusing a key returns the first result without another graph write; with `--all`, Hermes appends each task id to the batch key. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
The decomposer refuses cards with any live task in their dependency-parent chain and enforces a hard maximum of 15 children per graph (normal graphs should remain in the 2–6 range). Automatic and dashboard decompositions supply deterministic keys for you. Direct CLI and API callers must supply one explicitly.
...
| `POST` | `/tasks/:id/decompose` | Run the kanban decomposer — auxiliary LLM produces a task graph and the helper atomically creates the children + links the root + flips `triage → todo`. Requires JSON `{ "idempotency_key": "<stable-key>", "author": "<optional>" }`; reusing the key is a no-op that returns the first child set. Returns `{ok, task_id, reason, fanout, child_ids, new_title}`. Same 200-on-LLM-error convention as `/specify`. |
...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or has a recent PR-custody sighting (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).

`active_pr` is **ownership-aware**: a PR URL in a comment is not by itself proof that this task opened the PR. Workers are asked to cross-reference companion PRs on related cards, so the guard distinguishes custody from citation:

- **Declared** — the PR came from the task itself, by either signal: its own work product (a run summary, run metadata, run error, or `tasks.result`) names it, **or** its own assigned worker posted it in a comment. Always guards.
- **Referenced** — the URL was cross-posted onto this task by a different profile. If a *different* task has declared the same PR, the URL is a citation and does **not** guard; the dispatcher emits a `respawn_guard_pr_ignored` event naming the real owner. If no task anywhere has declared it, the conservative behaviour is preserved and it still guards.

The self-authorship half matters as much as the work-product half. Most workers announce their PR only in a comment and never mirror it into a run summary, so treating every comment as a citation would strip the guard from tasks that really do hold an open PR. The discriminator is *who wrote the comment* (`task_comments.author` vs `tasks.assignee`), never what it says — handoff prose and cross-reference prose are indistinguishable in practice.

Ordinary `active_pr` is deliberately a conservative, no-network custody lease rather than a remote GitHub status check. It lasts 24 hours from the latest authoritative sighting and does not infer `OPEN`, `CLOSED`, or `MERGED` from comment prose; consequently, a recently sighted PR that is already merged still guards until that lease expires. The separately authorized continuation path performs bounded remote state and exact-head verification before allowing another writer.

Whenever the guard does fire, the `respawn_guarded` event payload carries `pr_url`, `source_comment_id`, `ownership`, and `expires_at`, so `hermes kanban tail <id>` explains the freeze without reading the database. `source_comment_id` is `null` when the authoritative in-window sighting came only from run/result work product.
...
| `respawn_guarded` | `{reason}`, plus `{pr_url, pr_urls, ownership, source_comment_id, expires_at, window_seconds, ignored_pr_urls}` when the reason is `active_pr` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (this task has a PR-custody sighting in the last 24 h — a prior worker already produced or announced a PR). For `active_pr` the payload names the exact URL, the source comment when one exists (`source_comment_id` is `null` for run/result-only custody), whether ownership was `declared` (the task's own work product or its own worker's comment) or `referenced` (cross-posted by another profile and unclaimed elsewhere), and the timestamp the local custody lease lapses. Ordinary dispatch does not probe remote PR status; exact remote state/head verification is reserved for authorized continuation. The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
| `respawn_guard_pr_ignored` | `{reason, ignored_pr_urls}` | The `active_pr` guard inspected PR URLs on this task and let the spawn through because each one was declared by a *different* task — the mention is a cross-reference, not custody. `ignored_pr_urls` lists `{pr_url, declared_by}` pairs. Rate-limited to once per 24 h window per task so a 2-minute dispatcher tick cannot flood the ledger. Emitted so that "the guard looked and let it through" is as visible as a suppression. |
