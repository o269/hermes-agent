...
class Client:
    ...
    def create_task(self, *, title, assignee=None, status="running", body=None,
        ...
        return self._request("create_task", {
            ...
            "reasoning_effort": reasoning_effort, "id": id,
    ...
    def set_status(self, task_id, status, *, op_id=None):
        ...
        The legacy boardd-native mutation only changed the task row and leaked a
        manual ``seat-sticky`` claim when returning work to ``ready``. Execute
        the canonical transaction over a BrokerConnection instead. ``op_id`` is
        retained for source compatibility; this idempotent authority deliberately
        bypasses the obsolete native operation and its applied-ops ledger.
        """
        del op_id
