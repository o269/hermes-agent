...
# Mirrors the #99 board-liveness contract. This module is part of the installed
# ``hermes_cli`` package; repository-only ``scripts`` modules are not importable
# from sealed/wheel installs. Unknown statuses intentionally remain live.
_TERMINAL_BOARD_STATUSES = frozenset({
    "done",
    "archived",
    "completed",
    "cancelled",
...
def _is_live_board_status(status_value: str) -> bool:
    return (status_value or "").strip().lower() not in _TERMINAL_BOARD_STATUSES
...
class StageArtifacts:
    ...
    restricted_board_live_ids_path: Path
...
@dataclass(frozen=True)
class BoardLivenessSnapshot:
    board_path: Path
    board_identity: os.stat_result
    tasks_total: int
    linked_tasks: int
    live_linked_tasks: int
    live_session_ids: tuple[str, ...]
    task_projection_sha256: str
    ...
    def public_receipt(self) -> dict[str, Any]:
        return {
            ...
            "live_session_ids": len(self.live_session_ids),
            "live_session_ids_sha256": _ids_digest(self.live_session_ids),
            "task_projection_sha256": self.task_projection_sha256,
            "unknown_statuses_are_live": True,
        }
...
def _require_current_owner(info: os.stat_result, label: str) -> None:
    ...
    if get_effective_uid is None:
        raise ColdArchiveError(f"{label} ownership cannot be proven on this platform")
    if info.st_uid != get_effective_uid():
        raise ColdArchiveError(f"{label} must be owned by the current effective user")
...
def _create_private_dir(path: Path) -> Path:
    ...
    _require_current_owner(target.lstat(), f"new private directory {target}")
...
def _require_private_dir(path: Path) -> Path:
    ...
    _require_current_owner(info, f"stage directory {raw}")
...
def _require_private_file(path: Path) -> Path:
    ...
    _require_current_owner(info, f"stage artifact {raw}")
    if info.st_nlink != 1:
        raise ColdArchiveError(f"stage artifact must not have hardlink aliases: {raw}")
    return raw.resolve(strict=True)
    ...
    *,
    ...
    try:
        ...
        if require_current_owner:
            _require_current_owner(before, label)
...
def _stage_paths(root: Path) -> StageArtifacts:
    ...
    return StageArtifacts(
        ...
        restricted_board_live_ids_path=restricted / "board-live-session-ids.json",
...
def _protected_live_filesystem_devices() -> frozenset[int]:
    """Devices on which a default/current/named-profile DB alias could appear."""

    devices: set[int] = set()
    for protected in _candidate_live_paths():
        anchor = protected.parent
        try:
            while not anchor.exists() and anchor != anchor.parent:
                anchor = anchor.parent
            devices.add(anchor.resolve(strict=True).stat().st_dev)
        except OSError as exc:
            raise ColdArchiveError(
                f"could not prove protected profile filesystem boundary: {exc}"
            ) from exc
    if not devices:
        raise ColdArchiveError("no protected profile filesystem boundary was provable")
    return frozenset(devices)


def _require_candidate_filesystem_boundary(source: Path) -> None:
    source_info = source.stat()
    if source_info.st_dev in _protected_live_filesystem_devices():
        raise ColdArchiveError(
            "offline candidate must be on a different filesystem from every "
            "active/default/named-profile database path"
        )
...
def reject_live_state_db(source_path: Path) -> Path:
    ...
    for candidate_component in source_family:
        candidate_info = candidate_component.lstat()
        _require_current_owner(
            candidate_info, f"offline candidate {candidate_component}"
        )
        if candidate_info.st_nlink != 1:
            raise ColdArchiveError(
                f"offline candidate must not have hardlink aliases: {candidate_component}"
            )
...
def _require_archive_schema(conn: sqlite3.Connection) -> None:
    ...
    if _table_exists(conn, "state_meta"):
        rebuild_markers = [
            str(row[0])
            for row in conn.execute(
                """
        ...
        if rebuild_markers:
            raise ColdArchiveError(
                "cold archive refuses a candidate with an incomplete FTS rebuild: "
                + ", ".join(rebuild_markers)
            )
...
def _read_board_liveness_snapshot(
    conn: sqlite3.Connection, board_path: Path, board_identity: os.stat_result
) -> BoardLivenessSnapshot:
    table = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='tasks'"
    ).fetchone()
    if table is None:
        raise ColdArchiveError("board liveness control failed: tasks table is missing")
    columns = {
        str(row[1]) for row in conn.execute('PRAGMA table_info("tasks")').fetchall()
    }
    missing = sorted({"id", "status", "session_id"} - columns)
    if missing:
        raise ColdArchiveError(
            "board liveness control failed: tasks schema is missing "
            + ", ".join(missing)
        )
    rows = conn.execute(
        "SELECT id, status, session_id FROM tasks ORDER BY id"
    ).fetchall()
    tasks_total = int(conn.execute("SELECT COUNT(*) FROM tasks").fetchone()[0])
    if tasks_total <= 0 or len(rows) != tasks_total:
        raise ColdArchiveError(
            "board liveness control failed: a readable non-empty tasks population "
            "was not proven"
        )
    ...
    live_linked_tasks = 0
    for raw in rows:
        raw_task_id, raw_status, raw_session_id = raw[0], raw[1], raw[2]
        if (
            ...
            raise ColdArchiveError(
                "board liveness control failed: task id/status is unprovable"
            )
        task_id = raw_task_id
        ...
        if raw_session_id is not None:
            if (
                ...
                raise ColdArchiveError(
                    "board liveness control failed: linked session_id is unprovable"
                )
            session_id = raw_session_id
        projection.append([task_id, status_value, session_id])
        if session_id:
            linked_tasks += 1
            if _is_live_board_status(status_value):
                live_linked_tasks += 1
                live_session_ids.add(session_id)
    digest = _sha256_bytes(
        json.dumps(projection, sort_keys=True, separators=(",", ":")).encode("utf-8")
    )
    return BoardLivenessSnapshot(
        board_path=board_path,
        board_identity=board_identity,
        tasks_total=tasks_total,
        linked_tasks=linked_tasks,
        live_linked_tasks=live_linked_tasks,
        live_session_ids=tuple(sorted(live_session_ids)),
        task_projection_sha256=digest,
    )
...
def _assert_board_path_identity(snapshot: BoardLivenessSnapshot) -> None:
    try:
        ...
        raise ColdArchiveError(
            f"board database path changed during archive authorization: {exc}"
        ) from exc
    if not os.path.samestat(snapshot.board_identity, current):
        raise ColdArchiveError(
            "board database path identity changed during archive authorization"
        )


@contextmanager
def _board_liveness_snapshot(
    board_db: Path, *, reserve_writes: bool
) -> Iterator[BoardLivenessSnapshot]:
    ...
    board_identity = board_path.lstat()
    mode = "rw" if reserve_writes else "ro"
    uri = board_path.as_uri() + f"?mode={mode}"
    try:
        conn = sqlite3.connect(uri, uri=True, isolation_level=None, timeout=1.0)
    except sqlite3.DatabaseError as exc:
        raise ColdArchiveError(
    ...
    try:
        conn.row_factory = sqlite3.Row
        conn.execute("BEGIN IMMEDIATE" if reserve_writes else "BEGIN")
        conn.execute("PRAGMA query_only=ON")
        snapshot = _read_board_liveness_snapshot(conn, board_path, board_identity)
        ...
        if conn.in_transaction:
            try:
                ...
                pass
        conn.close()
        raise ColdArchiveError(f"board liveness query failed closed: {exc}") from exc
    except BaseException:
        if conn.in_transaction:
            try:
                conn.execute("ROLLBACK")
...
def _require_no_live_board_selection(
    snapshot: BoardLivenessSnapshot, selected_ids: Sequence[str]
) -> None:
    overlap = set(selected_ids).intersection(snapshot.live_session_ids)
    if overlap:
        raise ColdArchiveError(
            "approved selected set gained a non-terminal board task reference"
        )


def _validate_board_liveness_receipt(
    receipt: Any, *, restricted_ids: Sequence[str]
) -> None:
    if not isinstance(receipt, dict):
        raise ColdArchiveError("approved manifest lacks board liveness evidence")
    tasks_total = receipt.get("tasks_total")
    ...
    if not (
        tasks_total > 0
        and 0 <= linked_tasks <= tasks_total
        and 0 <= live_linked_tasks <= linked_tasks
        and 0 <= live_session_count <= live_linked_tasks
    ):
    ...
    if (
        ...
        or not re.fullmatch(
            r"[0-9a-f]{64}", str(receipt.get("task_projection_sha256") or "")
        )
    ):
        raise ColdArchiveError(
...
def build_gate_b_manifest(
    ...
    *,
    ...
    with _board_liveness_snapshot(board_db, reserve_writes=False) as board_snapshot:
        board_live_session_ids = set(board_snapshot.live_session_ids)
    ...
    with _connect_readonly(source) as conn:
        ...
        for index, component in enumerate(components, start=1):
            ...
            if board_live_session_ids.intersection(component):
                reasons.add("live_board_task_reference")
        ...
        manifest: dict[str, Any] = {
            ...
            "source_owned_single_link": True,
            "source_filesystem_separated_from_live_profiles": True,
            "board_liveness": board_snapshot.public_receipt(),
            "schema_contract": {
        ...
        return {
            **manifest,
            ...
            "_restricted_live_board_session_ids": sorted(board_live_session_ids),
        }
...
def write_gate_b_manifest(stage_root: Path, manifest: dict[str, Any]) -> StageArtifacts:
    ...
    board_ids = [
        str(sid) for sid in manifest.get("_restricted_live_board_session_ids") or []
    ]
    _exclusive_write_json(
        stage.restricted_board_live_ids_path,
        {"live_board_session_ids": board_ids},
    )
    _exclusive_write_json(
...
def load_approved_gate_b_manifest(
    ...
    *,
    ...
    board_ids_payload = _load_private_json(stage.restricted_board_live_ids_path)
    raw_board_ids = board_ids_payload.get("live_board_session_ids")
    if not isinstance(raw_board_ids, list) or any(
        not isinstance(value, str) for value in raw_board_ids
    ):
        raise ColdArchiveError("restricted board-live session IDs artifact is invalid")
    if len(raw_board_ids) != len(set(raw_board_ids)):
        raise ColdArchiveError("restricted board-live session IDs contain duplicates")
    board_ids = sorted(raw_board_ids)
    _validate_board_liveness_receipt(
        manifest.get("board_liveness"), restricted_ids=board_ids
    )
    ...
    return stage, {
        **manifest,
        ...
        "_restricted_parent_map": parent_map,
        "_restricted_live_board_session_ids": board_ids,
    }
...
def _build_restricted_packet(stage: StageArtifacts, qmd_report: dict[str, Any]) -> Path:
    members = [
        ...
        _require_private_file(stage.restricted_board_live_ids_path),
    ] + [
...
def _verify_fts_counts(
    ...
    for table in roots:
        ...
        if schema_row is None or not schema_row[0]:
            raise ColdArchiveError(f"{table} schema is unavailable")
        content_match = re.search(
            r"\bcontent\s*=\s*['\"]([^'\"]+)['\"]", str(schema_row[0]), re.I
        )
        if content_match:
            ...
            if not _table_exists(conn, source_table):
                raise ColdArchiveError(
                    f"{table} external content source is unavailable: {source_table}"
                )
            expected = _count_where(
                conn, f"SELECT COUNT(*) FROM {_quote_identifier(source_table)}"
            )
        else:
            # Legacy inline-content FTS roots index every message row.
            expected = message_count
        # FTS5 exposes its strict external-content verifier as a special INSERT,
        # so it cannot run through the query-only replay connections below.  Run
        # it while the destructive BEGIN IMMEDIATE transaction is still open;
        # the committed/prepared receipt is then bound to that verified state.
        if conn.in_transaction:
            try:
                conn.execute(
                    f"INSERT INTO {_quote_identifier(table)}"
                    f"({_quote_identifier(table)}, rank) VALUES ('integrity-check', 1)"
                )
            except sqlite3.DatabaseError as exc:
                raise ColdArchiveError(
                    f"{table} strict content integrity check failed"
                ) from exc
...
def _apply_retention_to_candidate_locked(
    candidate_db: Path,
    manifest: dict[str, Any],
    *,
    board_snapshot: BoardLivenessSnapshot,
        ...
        try:
            ...
            report = {
                ...
                **verified,
                ...
                "board_liveness_reverified": board_snapshot.public_receipt(),
                "board_writer_reservation_held_through_commit": True,
            }
            prepared = {
            ...
            # COMMIT. The cross-device fence makes a new hardlink alias
            # impossible; the board writer reservation prevents new live links.
            reject_live_state_db(source)
            _require_candidate_filesystem_boundary(source)
            _assert_board_path_identity(board_snapshot)
            _require_no_live_board_selection(board_snapshot, ids)
            _validate_candidate_path_identity(source, manifest)
            conn.execute("COMMIT")
        except BaseException:
...
def apply_retention_to_candidate(
    ...
    *,
    ...
    """Delete only while board liveness is stable under a writer reservation."""

    with _board_liveness_snapshot(board_db, reserve_writes=True) as board_snapshot:
        report = _apply_retention_to_candidate_locked(
            candidate_db,
            manifest,
            ...
            remote_custody_reverified=remote_custody_reverified,
        )
        return {
            **report,
            "board_liveness_reverified": board_snapshot.public_receipt(),
...
def _validate_candidate_matches_manifest(
    ...
    info = source.stat()
    _require_current_owner(info, f"offline candidate {source}")
    if info.st_nlink != 1:
        raise ColdArchiveError("offline candidate gained a hardlink alias")
    _require_candidate_filesystem_boundary(source)
...
def _retention_receipt_payload(
    ...
    *,
    ...
    return {
        ...
        "board_liveness_reverified": report.get("board_liveness_reverified"),
        "board_writer_reservation_held_through_commit": report.get(
            "board_writer_reservation_held_through_commit"
        ),
        "retention": report,
...
def _recover_committed_prepared_apply(
    ...
    *,
    ...
    if (
        ...
        or prepared.get("board_writer_reservation_held_through_commit") is not True
        or not isinstance(prepared.get("board_liveness_reverified"), dict)
    ):
    ...
    if report.get(
        "board_writer_reservation_held_through_commit"
    ) is not True or report.get("board_liveness_reverified") != prepared.get(
...
def _validate_final_retention_receipt(
    ...
    *,
    ...
    if (
        ...
        or receipt.get("gate_b_manifest_sha256") != manifest.get("manifest_sha256")
        or not isinstance(receipt.get("board_liveness_reverified"), dict)
    ):
    ...
    if (
        ...
        or report.get("board_writer_reservation_held_through_commit") is not True
        ...
        if apply_retention:
            ...
            if not ids:
                ...
                receipt = {
                    ...
                    "board_liveness_reverified": board_receipt,
                    "retention": {
                        **report,
                        "board_liveness_reverified": board_receipt,
                    },
