...
def _guard_non_default_update_branch(
    git_cmd: list[str],
    cwd: Path,
    target_branch: str,
    *,
    ...
    """Refuse an implicit checkout away from a non-default live branch.

    The fleet runs reviewed integration branches that intentionally diverge from
    ``main``. ``hermes update`` historically defaulted to ``main`` regardless of
    the active checkout, so an implicit update could autostash the live tree and
    move the running CLI onto incompatible code. An explicit ``--branch`` that
    names the branch already checked out is safe; moving to another branch needs
    the operator-only emergency override and a cutover plan.
    """
    if os.environ.get("HERMES_ALLOW_UPDATE") == "1":
        return
    result = subprocess.run(
    ...
    if result.returncode != 0:
        print("✗ Could not determine the active branch. Refusing to update safely.")
        sys.exit(1)
    current_branch = result.stdout.strip()
    if not current_branch or current_branch == "main":
        return
    if branch_explicit and target_branch == current_branch:
        return
    print(
        f"✗ Refusing to leave non-default checkout '{current_branch}' during update."
    )
    print("  Pass --branch with the active branch to update it in place.")
    print("  A deliberate branch move requires HERMES_ALLOW_UPDATE=1 and a cutover plan.")
    sys.exit(1)
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    target_remote, _target_url = _resolve_update_remote(git_cmd, _m().PROJECT_ROOT)
    fetch_remote = target_remote
    if branch == "main" and _has_upstream_remote(git_cmd, _m().PROJECT_ROOT):
        if _official_upstream_remote_is_safe(git_cmd, _m().PROJECT_ROOT):
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Resolve the branch and refuse any implicit departure from a non-default
    # live checkout before network, stash, checkout, or reset operations.
    branch = _m()._resolve_update_branch(args)
    ...
    # used. The ancestry fence below—not remote naming alone—protects committed
    # work before any destructive reset.
    target_remote, target_url = _resolve_update_remote(git_cmd, _m().PROJECT_ROOT)
    ...
    try:
        ...
        if current_branch != branch:
            ...
            checkout_result, local_branch_existed = _checkout_update_branch(
                git_cmd, _m().PROJECT_ROOT, branch, target_remote
            )
            if checkout_result.returncode != 0:
        ...
        try:
            ...
            pull_result = subprocess.run(
                git_cmd + ["merge", "--ff-only", f"{target_remote}/{branch}"],
                cwd=_m().PROJECT_ROOT,
                capture_output=True,
                text=True, encoding="utf-8", errors="replace",
            )
            if pull_result.returncode != 0:
                # A reset after a failed fast-forward is permitted only when we
                # can prove HEAD has no commits absent from the resolved target.
                # This is the committed-work fence from PR #10 adapted to the
                # extracted ``update_cmd.py`` live branch.
                ...
                if local_ahead > 0:
                    ...
                    print("  Push or reconcile the local commits before retrying.")
                    print(
